#!/usr/bin/env python3
"""Generate Traditional Chinese HTML report from real LLM predictions."""
...
PRED_ZH = {'HOME': '主勝', 'DRAW': '平局', 'AWAY': '客勝'}
CONS_ZH = {'high': '高', 'medium': '中', 'low': '低'}
ROLE_ZH = {'Fan': '球迷', 'Analyst': '分析師', 'Media': '媒體', 'Insider': '內幕', 'Neutral': '中立'}

def format_form(f):
    if not f or f == '?': return '<span class="form">-</span>'
    MAP = {'W': '勝', 'D': '平', 'L': '負'}
    return '<span class="form">' + ''.join(f'<span class="{c}">{MAP.get(c, c)}</span>' for c in f) + '</span>'
...
<title>MiroFish - 英超 2025/26 AI 預測報告</title>
<style>
*{{margin:0;padding:0;box-sizing:border-box}}
body{{font-family:'Noto Sans TC','Microsoft JhengHei','Segoe UI',-apple-system,sans-serif;background:#0a0a0a;color:#e0e0e0;line-height:1.6}}
...
.stats .l{{font-size:0.75em;color:#8b949e;margin-top:6px;letter-spacing:1px}}
...
<body>
<div class="header">
<h1>MiroFish <span class="accent">足球預言家</span></h1>
<div class="sub">英超 2025/26 賽季 AI 預測報告</div>
<div class="sub">由真實 AI 智能體集群分析驅動（Claude Haiku 4.5）</div>
<div class="badge">5 個 AI 智能體 x 79 場比賽 = 395 次 LLM 調用</div>
</div>

<div class="stats">
<div class="s home"><div class="n">{hw}</div><div class="l">主勝</div></div>
<div class="s draw"><div class="n">{dr}</div><div class="l">平局</div></div>
<div class="s away"><div class="n">{aw}</div><div class="l">客勝</div></div>
<div class="s total"><div class="n">{total}</div><div class="l">總場次</div></div>
<div class="s conf"><div class="n">{avg_conf:.0%}</div><div class="l">平均信心度</div></div>
</div>

<div class="container">
<div class="method">
<h3>雙層預測架構</h3>
<p><strong>第一層 - 機器學習特徵（動態權重）：</strong>聯賽排名、積分、近期狀態、淨勝球、攻防比率、主場優勢係數。</p>
<p><strong>第二層 - AI 智能體集群（動態權重）：</strong>5 個專業 AI 智能體（球迷 / 分析師 / 媒體 / 內幕 / 中立）各自透過 Claude Haiku 4.5 獨立分析比賽，然後投票表決。結果按權重擴展至 50 票（5+15+8+12+10）。</p>
<p><strong>動態融合：</strong>當智能體共識度為「高」時，智能體權重提升至 70%；共識度為「低」時，ML 權重提升至 55%。預設比例：ML 40% / 智能體 60%。</p>
<p style="margin-top:8px;color:#e94560;font-size:0.85em;">將滑鼠移到任何比賽卡片上，即可查看各 AI 智能體的詳細推理過程。</p>
</div>
"""

for md in sorted(matchdays.keys()):
    matches = matchdays[md]
    html += f'<div class="md-header">第 {md} 輪（{len(matches)} 場比賽）</div>\n'
    ...
    for m in sorted(matches, key=lambda x: x['date']):
        ...
        for ar in m.get('agent_reasonings', []):
            ...
            role_zh = ROLE_ZH.get(ar['role'], ar['role'])
            res_zh = PRED_ZH.get(res, res)
            reasons_html += f'<div class="r"><span class="role">{role_zh}</span> <span class="res {res}">{res_zh}</span> {reasoning}</div>\n'

        pred_zh = PRED_ZH.get(m['prediction'], m['prediction'])
        cons_zh = CONS_ZH.get(m['consensus'], m['consensus'])

        html += f"""<div class="card">
<div class="match">
  <div class="date-col">{date_str}<br>{time_str} UTC</div>
  <div class="team home">
    <div><div class="nm">{escape(m['home_team'])}</div><div class="meta">#{m['home_pos']} {m['home_pts']}分 {format_form(m.get('home_form',''))}</div></div>
  ...
  <div class="team away">
    ...
    <div><div class="nm">{escape(m['away_team'])}</div><div class="meta">#{m['away_pos']} {m['away_pts']}分 {format_form(m.get('away_form',''))}</div></div>
  </div>
  <div class="pred-badge {m['prediction']}">{pred_zh}</div>
  <div class="bar-wrap">
  ...
  <div class="cons {m['consensus']}">{cons_zh}</div>
</div>
...
html += f"""</div>
<div class="footer">
<p>MiroFish 足球預言家 | 雙層架構：機器學習 + AI 智能體集群投票</p>
<p>5 個 AI 智能體（球迷/分析師/媒體/內幕/中立）透過 Claude Haiku 4.5 分析 | 數據來源：football-data.org</p>
<p style="margin-top:8px">產生時間 {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC | {total} 場比賽 | {high}/{total} 場高共識</p>
<p style="margin-top:6px;color:#30363d">本預測僅供娛樂及研究用途，不構成任何投注建議。</p>
</div>
</body></html>"""
...
print(f"報告已產生：{out}")
print(f"{total} 場比賽 | {hw} 主勝 / {dr} 平局 / {aw} 客勝 | 平均信心度：{avg_conf:.0%} | 高共識：{high}/{total}")
